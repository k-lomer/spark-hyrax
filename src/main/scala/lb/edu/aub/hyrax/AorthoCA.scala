...
object AorthoCA {
  /* aorthoCABCGS - Communication Avoiding Block Classical Gram-Schmidt A-orthonormalization
  ...
  */
  def aorthoCABCGS( Q: DistributedDenseMatrix,  AQ: DistributedDenseMatrix,
  ...
  (DistributedDenseMatrix, DistributedDenseMatrix) = {
    ...
    if(CGS2) {
      val ret = aorthoCABCGS(Q, AQ, scaledNewW, scaledNewAW, false, cache)
      (ret._1, ret._2)
    }
